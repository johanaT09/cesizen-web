<template>
    <div class="bg-backgroundPrimary min-h-screen py-12 md:py-20 px-6 sm:px-12 lg:px-24">
        ...
        <div class="max-w-7xl mx-auto bg-textSecondary p-6 rounded-[24px] shadow-sm border border-textPrimary/5 mb-10">
            <div class="flex flex-col lg:flex-row items-center gap-4 w-full">
                ...
                <div class="relative w-full lg:w-72">
                    ...
                    <div
                        class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-textPrimary/40">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-textPrimary/5 font-body text-sm">
                        <tr v-if="loading" class="text-center">
                            <td colspan="6" class="px-8 py-16 text-textPrimary/40 italic text-base">Chargement du
                                catalogue...</td>
                        </tr>
                        <tr v-else-if="activities.length === 0" class="text-center">
                            <td colspan="6" class="px-8 py-16 text-textPrimary/50 text-base">Aucune activité
                                enregistrée.</td>
                        </tr>
                        <tr v-else v-for="act in activities" :key="act.id_activite"
                            ...
                            <td class="px-6 py-5">
                                <span
                                    class="px-3 py-1 rounded-full bg-textVert/10 text-textVert font-bold uppercase text-[11px] font-body">
                            ...
                            <td class="px-6 py-5">
                                <span
                                    ...
                                    :class="act.est_actif ? 'bg-textVert/10 text-textVert' : 'bg-red-50 text-red-600 bg-red-50'">
        ...
        <div v-if="totalPages > 1" class="max-w-7xl mx-auto mt-12 flex justify-center items-center gap-4">
            ...
            <span class="text-sm font-medium text-textPrimary/60 font-body">Page {{ currentPage }} sur {{ totalPages
            }}</span>
        ...
        <div v-if="showModal"
            ...
            <div
                class="bg-white rounded-[32px] p-8 w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-200 overflow-y-auto max-h-[90vh]">
                ...
                <form @submit.prevent="saveActivite" class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div
                            class="flex items-center gap-4 p-4 bg-backgroundPrimary/50 rounded-xl border border-textPrimary/10">

                            <div
                                class="w-20 h-20 rounded-xl bg-gray-100 overflow-hidden flex items-center justify-center border border-textPrimary/5 flex-shrink-0">
                                <img v-if="imagePreview" :src="imagePreview" class="w-full h-full object-cover" />
                                <span v-else
                                    class="text-[10px] text-textPrimary/30 font-bold uppercase font-body tracking-wider">Aucune</span>
                            </div>

                            <div class="flex flex-col gap-1">
                                <input type="file" id="cover_image" @change="onFileChange" accept="image/*"
                                    class="hidden" />
                                <label for="cover_image"
                                    class="px-4 py-2 text-xs font-bold text-textPrimary bg-textSecondary border border-textPrimary/10 rounded-lg shadow-sm hover:bg-backgroundPrimary cursor-pointer transition inline-block text-center w-fit font-body">
                                    Sélectionner un fichier
                                </label>
                                <span class="text-[11px] text-textPrimary/40 font-body">Extensions acceptées : PNG, JPG,
                                    WEBP (Max. 2 Mo)</span>
                            </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                <select v-model="form.id_type" required
                                    class="w-full px-4 py-3 rounded-xl bg-backgroundPrimary border border-textPrimary/10 focus:border-buttonPrimary outline-none text-sm text-textPrimary appearance-none font-body">
                                    <option :value="null" disabled>Sélectionner</option>
                                    <option v-for="t in types" :key="t.id_type" :value="t.id_type">{{ t.libelle_type ||
                                        'Type ' + t.id_type }}</option>
                                </select>
                                <div
                                    class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-textPrimary/40">
                                    <BaseIcon name="chevron-down" customClass="h-4 w-4" />
                    ...
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-textPrimary">Fichier Vidéo de l'exercice
                            (Optionnel)</label>
                        ...
                        <div class="p-4 bg-backgroundPrimary/50 rounded-xl border border-textPrimary/10">
                            ...
                            <p class="text-[11px] text-textPrimary/40 font-body mt-2">
                                ...
                                <span v-if="isEditing && form.lien_ressource"><br>Sélectionnez un nouveau fichier
                                    uniquement pour remplacer la vidéo actuelle.</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div v-if="isEditing" class="flex items-center gap-3 pt-2">
                        ...
                        <label for="act_actif"
                            class="text-sm font-bold text-textPrimary cursor-pointer font-body">Activité en ligne
                            (disponible pour les utilisateurs)</label>
                    </div>
