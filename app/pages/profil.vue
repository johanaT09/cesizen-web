...
fill-rule="evenodd"
        ...
        <button
type="button" class="text-textPrimary/40 hover:text-textPrimary/70 transition-colors p-1"
    ...
    <div
      ...
      <section class="space-y-6" aria-labelledby="title-infos">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-50 pb-2">
          ...
          <button
type="button" :aria-expanded="isEditingInfo" :class="isEditingInfo ? 'bg-gray-100 text-textPrimary hover:bg-gray-200' : 'bg-buttonPrimaryDegrade1 text-white hover:opacity-90'"
        ...
        <form class="space-y-4" @submit.prevent="submitUpdate('info')">
...
for="prenom"
              ...
              <input
v-if="isEditingInfo" id="prenom" v-model="form.prenom" type="text"
...
for="date_naissance"
                ...
                naissance</label>
              <input
v-if="isEditingInfo" id="date_naissance" v-model="form.date_naissance" type="date"
                class="w-full px-4 py-2.5 rounded-xl bg-backgroundPrimary border border-textPrimary/10 text-sm text-textPrimary outline-none focus:border-buttonPrimary focus:ring-2 focus:ring-buttonPrimary/20"
                required >
              <p v-else class="text-base font-medium text-textPrimary py-1.5 px-1">{{ displayDate }}</p>
              <p
v-if="isEditingInfo && errors.date_naissance" class="text-xs text-red-500 mt-1 font-medium"
...
for="id_genre"
              ...
              <select
v-if="isEditingInfo" id="id_genre" v-model="form.id_genre"
                class="w-full px-4 py-2.5 rounded-xl bg-backgroundPrimary border border-textPrimary/10 text-sm text-textPrimary outline-none focus:border-buttonPrimary focus:ring-2 focus:ring-buttonPrimary/20">
...
:disabled="loading" type="submit"
      ...
      <section class="space-y-6 pt-6 border-t border-gray-100" aria-labelledby="title-security">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-50 pb-2">
          ...
          <button
type="button" :aria-expanded="isEditingPassword" :class="isEditingPassword ? 'bg-gray-100 text-textPrimary hover:bg-gray-200' : 'bg-buttonPrimaryDegrade1 text-white hover:opacity-90'"
            class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all focus-visible:ring-2 focus-visible:ring-buttonPrimary outline-none active:scale-[0.98]"
...
v-if="errors.global"
        ...
        <form class="space-y-4" @submit.prevent="submitUpdate('password')">
...
for="current_password"
                ...
                actuel</label>
              <input
id="current_password" v-model="form.current_password" type="password" placeholder="••••••••"
                class="w-full px-4 py-2.5 rounded-xl bg-backgroundPrimary border border-textPrimary/10 text-sm text-textPrimary outline-none focus:border-buttonPrimary focus:ring-2 focus:ring-buttonPrimary/20"
                required >
...
for="new_password"
                  ...
                  passe</label>
                <input
id="new_password" v-model="form.new_password" type="password" placeholder="••••••••"
                  class="w-full px-4 py-2.5 rounded-xl bg-backgroundPrimary border border-textPrimary/10 text-sm text-textPrimary outline-none focus:border-buttonPrimary focus:ring-2 focus:ring-buttonPrimary/20"
                  required >
...
for="new_password_confirmation"
                  ...
                  mot de passe</label>
                <input
id="new_password_confirmation" v-model="form.new_password_confirmation" type="password"
                  placeholder="••••••••"
                  class="w-full px-4 py-2.5 rounded-xl bg-backgroundPrimary border border-textPrimary/10 text-sm text-textPrimary outline-none focus:border-buttonPrimary focus:ring-2 focus:ring-buttonPrimary/20"
                  required >
...
to="/accessibilite-rgaa"
      ...
      <section
        v-if="!isAdmin"
        class="space-y-4 pt-6 border-t border-gray-100 bg-red-50/20 -mx-6 md:-mx-10 px-6 md:px-10 pb-4 rounded-b-[32px]" aria-labelledby="title-danger">
        <div>
        ...
        <div
          ...
          <button
type="button" class="w-full sm:w-auto text-center px-5 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-sm font-bold border border-red-200 transition-all focus-visible:ring-2 focus-visible:ring-red-500 outline-none active:scale-[0.98] whitespace-nowrap"
    ...
    <ConfirmModal
:show="showDeleteModal" title="Supprimer mon compte"
