  ...
  <header class="sticky top-0 z-40 border-b border-gray-200 bg-navbar/90 backdrop-blur-md">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
      ...
      <NuxtLink to="/" class="flex items-center gap-3 transition-opacity hover:opacity-90">
        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-buttonPrimary textSecondary shadow-sm">
      ...
      <div class="hidden items-center gap-3 md:flex">
        ...
        <NuxtLink to="/inscription"
          class="rounded-full bg-buttonPrimary px-6 py-2.5 text-sm font-bold textSecondary shadow-md hover:brightness-110 transition-all active:scale-95">
          Créer un compte
        </NuxtLink>
      </div>
      ...
      <div v-if="isMenuOpen" 
        ...
        <nav class="flex flex-col gap-2 p-6 bg-white h-full">
          ...
          <div class="mt-8 grid grid-cols-1 gap-4">
            ...
            <NuxtLink to="/inscription"
              class="flex h-14 items-center justify-center rounded-xl bg-buttonPrimary font-bold textSecondary shadow-lg active:brightness-90">
              Créer un compte
            </NuxtLink>
          </div>
        </nav>
